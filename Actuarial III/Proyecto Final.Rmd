...
for(i in 1:n_sim){
sim_serie[i,1] <- rexp(1,lambda)
sim_serie[i,2] <- rexp(1,theta)
}

sim_serie <- data.frame(T_i=sim_serie[,1],
                        X_i=sim_serie[,2])

sim_serie <- sim_serie[order(sim_serie$T_i),] #Ordena la serie de tiempo


ggplot(sim_serie, aes(x=T_i,y=X_i)) + geom_point() + coord_cartesian(xlim=c(0,5))
```

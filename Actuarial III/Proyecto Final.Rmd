...
```{r}
sop_real <- c(0:100000)
real_X <- data.frame(soporte=sop_real,teorica=pexp(sop_real,theta))


ggplot(sim_serie,aes(X_i))+ stat_ecdf()+ geom_line(data=real_X,aes(x=sop_real,y=teorica),col="red")

#Percentiles
media_per <-length(sop_real)*(.10) 
var_per <- length(sop_real)*(.1)*(.9)
Int_conf_p <- c((-1.645)*sqrt(var_per)+media_per+.5,(1.645)*sqrt(var_per)-media_per+.5)
Int_conf_p
#percentiles_2.4 <- data.frame(i=sop_real,Pr_i=)
```
